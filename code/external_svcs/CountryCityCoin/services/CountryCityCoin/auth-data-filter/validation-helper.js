...
const { country_city_currency } = require('./filter-country-city-currency')
...
const validateName = (provider) => {
  throwExeptionIfNotHasProperty(provider, 'name', messageBinder().nameIsMissing)
  throwExeptionIfEmpty(provider.name, messageBinder().nameIsMissing)
}

const validateCountry = (aCountry) => {
  return validCountry(aCountry, messageBinder().countryIsInvalid)
}
...
const validateEmail = (provider) => {
  ...
  thrrowErrorIfNotValidEmail(provider.email, messageBinder().invalidEmailFormat)
}
...
module.exports = {
  ...
  validateCity,
  validateCucrrency,
  validateCucrrencySymbol,
  validateCucrrencyCode,
  validateEmail
}
